            ...
            <div class="modal-body px-3">
                <form class="row g-3 mb-3" action="{{ route('peminjaman.store') }}" method="POST">
                    ...
                    <div class="col-12">
                        ...
                        <select id="status_peminjaman" name="status_peminjaman" class="form-select">
                            <option selected disabled>-- Pilih Status --</option>
                            <option value="Dipinjam" {{ old('status_peminjaman') == 'Dipinjam' ? 'selected' : '' }}>
                                Dipinjam</option>
                            <option value="Dikembalikan"
                                {{ old('status_peminjaman') == 'Dikembalikan' ? 'selected' : '' }}>Dikembalikan
                            </option>
                            <option value="Diperpanjang"
                                {{ old('status_peminjaman') == 'Diperpanjang' ? 'selected' : '' }}>Diperpanjang
                            </option>
                            <option value="Hilang" {{ old('status_peminjaman') == 'Hilang' ? 'selected' : '' }}>Hilang
                            </option>
                        </select>
